...
class SF_Product extends SF_Resource
{
    ...
    private function get_product_data( $product )
    {
        ...
        {
            $mpn = get_post_meta( $product->id, '_model_number', true );
            if ( empty( $mpn ) )
            {
                $mpn = get_post_meta( $product->id, '_model_no', true );
            }
        }
        ...
        {
            $gtin = get_post_meta( $product->id, '_gtin', true );
            if ( empty( $gtin ) )
            {
                $gtin = get_post_meta( $product->id, '_upc', true );
            }
        }
        ...
        if  ($to = get_post_meta( $product->id, '_sale_price_dates_to', true ) )
        {
            $from = get_post_meta( $product->id, '_sale_price_dates_from', true );
...
//            'attributes'         => $attributes,
    ...
    private function get_variation_data( $product ) {
        ...
        foreach ( $product->get_children() as $child_id ) {
            ...
            $sale_price_effective_date = '';
            if  ($to = get_post_meta( $variation->get_variation_id(), '_sale_price_dates_to', true ) )
            {
                $from = get_post_meta( $variation->get_variation_id(), '_sale_price_dates_from', true );
                $sale_price_effective_date = ShoppingFeeder::format_datetime( $from ) . '/' . ShoppingFeeder::format_datetime( $to );
            }
